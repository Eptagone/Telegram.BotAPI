#:package Humanizer.Core@3.0.10
#:include constants.cs
#:include models.cs
#:package AngleSharp@1.7.2
...
static class Mapper
{
    ...
    public static IEnumerable<ClassDefinition> MapTypesIntoClasses(
        this BotApiDefinitions definitions
    )
    {
        ...
        foreach (var telegramTypeGroup in definitions.TypeGroups)
        {
            ...
            var commonPropertyGroups = types
                ...
                .Where(g =>
                    g.Count() == types.Count()
                    && g.Select(p => p.IsRequired).Distinct().Count() == 1
                )
            ...
            foreach (var propGroup in commonPropertyGroups)
            {
                ...
                var references = definitions
                    ...
                    .Select(s =>
                        string.Format("{0}.{1}", ScriptConstants.PROJECT_NAME, s.Pascalize())
                    );
    ...
    public static IEnumerable<ClassDefinition> MapMethodsIntoClasses(
        this BotApiDefinitions definitions
    )
    {
        ...
        foreach (var telegramMethod in definitions.Methods)
        {
            ...
            var hasComplexArguments =
                telegramMethod.Parameters.Any(p =>
                    p.ArrayLevels > 0
                    || p.Types.Any(t => char.IsUpper(AdjustTypeName(t, p.Name)[0]))
                )
            ...
            if (hasComplexArguments)
            {
                ...
                foreach (var param in telegramMethod.Parameters)
                {
                    ...
                    var parsedPropdescription = ParseDocReferences(description, definitions);
                    argsUsings.AddRange(
                        ExtractUsingsFromParsedDoc(parsedPropdescription, definitions)
                    );
            ...
            if (
                telegramMethod.Parameters.Any(p => p.Types.Any(t => HasAttachments(t, definitions)))
            )
            {
                usings.Add($"{ScriptConstants.PROJECT_NAME}.AvailableTypes");
            }
            ...
            foreach (var rt in telegramMethod.ReturnTypes)
            {
                var references = definitions
                    ...
                    .Concat(
                        definitions
                            .TypeGroups.Where(g => g.Name == rt.Type)
                            .Select(g => g.SectionName)
                    )
                    .Distinct()
                    .Select(s =>
    ...
    public static IEnumerable<ClassDefinition> MapConstantsIntoClasses(
        this BotApiDefinitions definitions
    )
    {
        var groups = new[]
    ...
    static bool HasAttachments(string typeName, BotApiDefinitions definitions, int deep = 10)
    {
        ...
        return group != null
            && deep > 0
            && group.Types.Any(t => HasAttachments(t, definitions, deep - 1));
    }

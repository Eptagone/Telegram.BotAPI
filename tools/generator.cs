...
{
    public static void GenerateClasses(IEnumerable<ClassDefinition> classes, string outputPath)
    {
        foreach (var @class in classes)
        {
            ...
            foreach (var method in @class.Methods)
            {
                foreach (var isAsync in new[] { false, true })
                {
                    ...
                    var returnTemplate =
                        method.ReturnType == "TResult" ? "<TResult>" : string.Empty;
                    builder.Append(
                        ...
                        if (method.Parameters.Any())
                        {
                            if (method.Parameters.First().Name != "args")
                            {
                                ...
                                if (argsClass == null)
                                {
                                    ...
                                    foreach (
                                        var param in method.Parameters.Where(p => p.IsRequired)
                                    )
                                    {
                                        ...
                                        {
                                            var propDef =
                                                $"{{ PropertyNames.{paramName}, {param.Name}";
            ...
            else
            {
                foreach (var prop in @class.Properties)
                {
                    ...
                    else
                    {
                        ...
                        {
                            builder.Append(
                                $"\tpublic {typeName} {WritePropName(prop.Name, @class)}"
                            );
                        }
            ...
            Console.Write("Writing file {0}...", filePath);

...
sealed class BotCommandExtractor : IBotCommandExtractor
{
    ...
    {
        var pattern = string.Format(CultureInfo.InvariantCulture, COMMAND_EXTRACT_PATTERN, username);
    ...
    {
        return !string.IsNullOrEmpty(message.Text) && this.cmdRegex.IsMatch(message.Text);
    }

    /// <inheritdoc/>
    public (string commandName, string args) ExtractCommand(Message message)
    {
        var match = this.cmdRegex.Match(message.Text);

	...
	where TBotProperties : IBotProperties
{
	...
	/// <inheritdoc/>
	public override async Task OnUpdateAsync(Update update, CancellationToken cancellationToken = default)
	...
	protected override async Task OnMessageAsync(Message message, CancellationToken cancellationToken = default)
	{
		if (!string.IsNullOrEmpty(message.Text))
		{
			...
			if (cmdMatch.Success)
			{
				await this.OnCommandAsync(message, cmdMatch.Name, cmdMatch.Params, cancellationToken);
			}
		}
	}
	...
	/// <param name="cancellationToken">Optional. The cancelation token.</param>
	/// <returns><see cref="Task"/></returns>
	protected abstract Task OnCommandAsync(Message message, string commandName, string commandParameters, CancellationToken cancellationToken = default);
}
...
public abstract class AsyncTelegramBotBase : ITelegramBot
{
	...
	/// <param name="cancellationToken">Optional. The cancelation token.</param>
	/// <returns><see cref="Task"/></returns>
	protected abstract Task OnMessageAsync(Message message, CancellationToken cancellationToken = default);
	...
	/// <param name="cancellationToken">Optional. The cancelation token.</param>
	/// <returns><see cref="Task"/></returns>
	protected virtual Task OnMyChatMemberAsync(ChatMemberUpdated myChatMemberUpdated, CancellationToken cancellationToken = default) => Task.CompletedTask;
	/// <summary>Instructions for chat member update.</summary>
	/// <param name="chatMemberUpdated">Chat member updated.</param>
	/// <param name="cancellationToken">Optional. The cancelation token.</param>
	/// <returns><see cref="Task"/></returns>
	protected virtual Task OnChatMemberAsync(ChatMemberUpdated chatMemberUpdated, CancellationToken cancellationToken = default) => Task.CompletedTask;
	...
	/// <param name="cancellationToken">Optional. The cancelation token.</param>
	/// <returns><see cref="Task"/></returns>
	protected abstract Task OnBotExceptionAsync(BotRequestException exp, CancellationToken cancellationToken = default);
	/// <summary>Instructions for a general exception.</summary>
	/// <param name="exp">Exception</param>
	/// <param name="cancellationToken">Optional. The cancelation token.</param>
	/// <returns><see cref="Task"/></returns>
	protected abstract Task OnExceptionAsync(Exception exp, CancellationToken cancellationToken = default);
}

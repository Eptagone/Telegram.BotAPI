...
public static partial class InlineModeExtensions
{
    ...
    /// <exception cref="BotRequestException">Thrown if the request to the Telegram Bot API fails.</exception>
    /// <returns></returns>
    public static Task<SentWebAppMessage> AnswerWebAppQueryAsync(this ITelegramBotClient client, CancellationToken cancellationToken = default)
    {
        if (client is null)
        {
            throw new ArgumentNullException(nameof(client));
        ...
        return client.CallMethodAsync<SentWebAppMessage>(MethodNames.AnswerWebAppQuery, cancellationToken: cancellationToken);
    }
}

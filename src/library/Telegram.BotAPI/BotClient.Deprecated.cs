...
public sealed partial class BotClient : TelegramBotClient
{
	...
	public BotResponse<T> GetRequest<T>(string method) => this.GetRequestAsync<T>(method).GetAwaiter().GetResult();
	...
	public Task<BotResponse<T>> GetRequestAsync<T>(string method, CancellationToken cancellationToken = default) => ((ITelegramBotClient)this).CallMethodDirectAsync<T>(method, cancellationToken: cancellationToken);
	...
	/// <returns><see cref="BotResponse{T}"/></returns>
	[Obsolete("Use IBotClient.CallApi<T> instead.")]
	public BotResponse<T> PostRequest<T>(string method, object args, [Optional] JsonSerializerOptions serializeOptions) => this.PostRequestAsync<T>(method, args, serializeOptions).Result;
	...
	/// <returns><see cref="BotResponse{T}"/></returns>
	[Obsolete("Use ITelegramBotClient.CallApiAsync<T> instead.")]
	public async Task<BotResponse<T>> PostRequestAsync<T>(string method, object args, [Optional] JsonSerializerOptions? serializeOptions, CancellationToken cancellationToken = default)
	{
		var stream = await Tools.SerializeAsStreamAsync(args, serializeOptions ?? SerializerOptions, cancellationToken).ConfigureAwait(false);
	...
	/// <returns><see cref="BotResponse{T}"/></returns>
	[Obsolete("Use ITelegramBotClient.CallApiAsync<T> instead.")]
	public Task<BotResponse<T>> PostRequestAsync<T>(string method, Stream args, CancellationToken cancellationToken = default) => ((ITelegramBotClient)this).CallMethodDirectAsync<T>(method, args, cancellationToken);
	...
	internal async Task<BotResponse<T>> SendRequestAsync<T>(HttpRequestMessage request, CancellationToken cancellationToken)
	{
		...
		try
		{
			...
			var botResponse = await JsonSerializer.DeserializeAsync<BotResponse<T>>(streamResponse, SerializerOptions, cancellationToken: cancellationToken);
			return botResponse!;
		}

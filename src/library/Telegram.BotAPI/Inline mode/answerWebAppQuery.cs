...
public static partial class InlineModeExtensions
{
	...
	public static SentWebAppMessage AnswerWebAppQuery(this ITelegramBotClient api, string webAppQueryId, InlineQueryResult result)
	{
		if (api == null) { throw new ArgumentNullException(nameof(api)); }
		var args = new AnswerWebAppQueryArgs(webAppQueryId, result);
		return api.CallMethod<SentWebAppMessage>(MethodNames.AnswerWebAppQuery, args);
	}
	...
	/// <exception cref="BotRequestException">Thrown when a request to Telegram Bot API got an error response.</exception>
	/// <exception cref="ArgumentNullException">Thrown when a required parameter is null.</exception>
	public static async Task<SentWebAppMessage> AnswerWebAppQueryAsync(this ITelegramBotClient api, string webAppQueryId, InlineQueryResult result, CancellationToken cancellationToken = default)
	{
		if (api == null) { throw new ArgumentNullException(nameof(api)); }
		var args = new AnswerWebAppQueryArgs(webAppQueryId, result);
		return await api.CallMethodAsync<SentWebAppMessage>(MethodNames.AnswerWebAppQuery, args, cancellationToken).ConfigureAwait(false);
	}
}

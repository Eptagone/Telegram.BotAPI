...
namespace HelloBotNET.AppService
{
	public class Worker(ILogger<Worker> logger, HelloBot bot) : BackgroundService
	{
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
		{
			...
			// Long Polling
			var updates = await client.GetUpdatesAsync(cancellationToken: stoppingToken).ConfigureAwait(false);
			while (!stoppingToken.IsCancellationRequested)
			{
				if (updates.Length != 0)
				{
					...
					updates = await client.GetUpdatesAsync(updates[^1].UpdateId + 1, cancellationToken: stoppingToken).ConfigureAwait(false);
				}
				else
				{
					updates = await client.GetUpdatesAsync(cancellationToken: stoppingToken).ConfigureAwait(false);
				}
			}
		}

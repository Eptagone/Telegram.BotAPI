...
namespace TelegramCalendar
{
	public static class Calendar
	{
		...
		public static IKB[][] New(Month mon)
		{
			...
			for (int i = 2; i < mon.Weeks + 2; i++)
			{
				...
				for (int j = 0; j < 7; j++)
				{
					if (pos < mon.Days.Length)
					{
						...
						{
							calendar[i][j] = new IKB($"{mon.Days[pos].Number}") { CallbackData = $"{mon.Days[pos].Name}, {mon.Name} {mon.Days[pos].Number}" };
							pos++;
						}
						else
						{
			...
			calendar[^1][0] = new IKB($"{previousmonth}") { CallbackData = $"month {previousyear} {(ushort)previousmonth}" };
			calendar[^1][1] = new IKB($"{nextmonth}") { CallbackData = $"month {nextyear} {(ushort)nextmonth}" };
			return calendar;
		}
		public static IKB[][] New(uint year)
		{
			...
			keyboard[0] = new IKB[1]{
				new IKB($"{year}") { CallbackData = $"Year {year}" }
			};
			for (int i = 1, n = 0; i < 5; i++)
			{
				keyboard[i] = new IKB[3];

...
public static partial class AvailableMethodsExtensions
{
	...
	public static bool SendChatAction(this BotClient? api, long chatId, string action, [Optional] int? messageThreadId)
	{
		...
		return api.RPC<bool>(MethodNames.SendChatAction, stream);
	}
	...
	/// <exception cref="BotRequestException">Thrown when a request to Telegram Bot API got an error response.</exception>
	/// <exception cref="ArgumentNullException">Thrown when a required parameter is null.</exception>
	public static bool SendChatAction(this BotClient? api, string chatId, string action, [Optional] int? messageThreadId)
	{
		if (api == null) { throw new ArgumentNullException(nameof(api)); }
		var stream = new MemoryStream();
		using var json = new Utf8JsonWriter(stream, new JsonWriterOptions { Indented = true });
	...
	/// <exception cref="BotRequestException">Thrown when a request to Telegram Bot API got an error response.</exception>
	/// <exception cref="ArgumentNullException">Thrown when a required parameter is null.</exception>
	public static async Task<bool> SendChatActionAsync(this BotClient? api, long chatId, string action, [Optional] int? messageThreadId, [Optional] CancellationToken cancellationToken)
	{
		if (api == null) { throw new ArgumentNullException(nameof(api)); }
		var stream = new MemoryStream();
		using var json = new Utf8JsonWriter(stream, new JsonWriterOptions { Indented = true });
		...
		stream.Seek(0, SeekOrigin.Begin);
		return await api.RPCA<bool>(MethodNames.SendChatAction, stream, cancellationToken).ConfigureAwait(false);
	}
	...
	/// <exception cref="BotRequestException">Thrown when a request to Telegram Bot API got an error response.</exception>
	/// <exception cref="ArgumentNullException">Thrown when a required parameter is null.</exception>
	public static async Task<bool> SendChatActionAsync(this BotClient? api, string chatId, string action, [Optional] int? messageThreadId, [Optional] CancellationToken cancellationToken)
	{
		if (api == null) { throw new ArgumentNullException(nameof(api)); }
		var stream = new MemoryStream();
		using var json = new Utf8JsonWriter(stream, new JsonWriterOptions { Indented = true });
